...
"""
Debug Forwarder
This script helps debug the Telegram to SMS forwarder service.
"""
    ...
    if config.ONLY_NON_MUTED_CHATS:
        try:
            ...
            if not found_dialog:
                ...
                # If we can't find the dialog, default to NOT monitoring it
                # This is a safety measure to prevent forwarding from unknown chats
                return False
                
        except Exception as e:
            logger.error(f"Error checking mute status: {e}")
            # If there's an error checking mute status, default to NOT monitoring the chat
            # This is a safety measure to prevent forwarding from unknown chats
            return False
...
async def debug_forwarder():
    ...
    await client.send_message('me', 'Test message from debug_forwarder.py')
