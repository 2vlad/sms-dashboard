#!/usr/bin/env python3
"""
Debug Forwarder Verbose
This script helps debug the Telegram to SMS forwarder service with very verbose logging.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("debug_verbose.log", mode='w'),
...
def save_message(user_id, chat_name, sender_name, message_text, delivered):
    ...
    conn.commit()
    conn.close()
    logger.debug(f"Saved message to database: user_id={user_id}, chat_name={chat_name}, delivered={delivered}")
...
def save_telegram_message(user_id, message_id, chat_id, chat_name, sender_name, message_text, timestamp, forwarded=True):
    ...
    conn.commit()
    conn.close()
    logger.debug(f"Saved telegram message to database: user_id={user_id}, message_id={message_id}, chat_id={chat_id}")
...
async def is_monitored_chat(client, chat_id):
    ...
    # Add debug logging
    logger.debug(f"Checking if chat {chat_id} should be monitored")
    
    # First, check if we should only monitor non-muted chats
    if config.ONLY_NON_MUTED_CHATS:
        try:
            # Get the dialog for this chat
            logger.debug(f"ONLY_NON_MUTED_CHATS is enabled, checking if chat {chat_id} is muted")
            dialogs = await client.get_dialogs(limit=100)  # Increase limit to find more chats
            found_dialog = False
            
            for d in dialogs:
                logger.debug(f"Checking dialog: {d.name} (ID: {d.entity.id})")
                if d.entity.id == chat_id:
                    found_dialog = True
                    # Check if the chat is muted
                    if d.dialog.notify_settings.mute_until:
                        logger.debug(f"Chat {chat_id} is muted, skipping")
                        return False
                    else:
                        logger.debug(f"Chat {chat_id} is not muted, will be monitored")
                    break
    ...
    if config.FORWARD_ALL_CHATS:
        logger.debug(f"FORWARD_ALL_CHATS is enabled, chat {chat_id} will be monitored")
        return True
    
    if not config.MONITORED_CHATS:
        logger.debug(f"No monitored chats configured, chat {chat_id} will not be monitored")
        return False
    
    # Check if chat_id is in the monitored chats list
    if chat_id in config.MONITORED_CHATS:
        logger.debug(f"Chat {chat_id} is in MONITORED_CHATS, will be monitored")
        return True
    ...
    try:
        ...
        if hasattr(entity, 'username') and entity.username in config.MONITORED_CHATS:
            logger.debug(f"Chat {chat_id} username is in MONITORED_CHATS, will be monitored")
            return True
    except Exception as e:
        logger.error(f"Error checking entity: {e}")
    
    logger.debug(f"Chat {chat_id} will not be monitored")
    return False
...
async def debug_forwarder():
    ...
    try:
        ...
        # Send a test SMS
        test_message = f"This is a test message from debug_forwarder_verbose.py at {time.strftime('%H:%M:%S')}"
    ...
    async def handle_new_message(event):
        try:
            # Add debug logging
            logger.debug(f"Received new message event: {event.id}")
            ...
            # Add debug logging
            logger.debug(f"Message from chat: {chat_id} ({get_display_name(chat)})")
            logger.debug(f"Chat details: {chat}")
            
            # Skip if this chat is not monitored
            monitored = await is_monitored_chat(client, chat_id)
            logger.debug(f"Is chat monitored: {monitored}")
            if not monitored:
    ...
    await client.send_message('me', 'Test message from debug_forwarder_verbose.py')
    ...
    # Print all dialogs to help debug
    logger.info("Listing all dialogs:")
    ...
    for i, dialog in enumerate(dialogs):
        ...
        logger.info(f"{i+1}. {dialog.name} (ID: {dialog.entity.id}, Type: {entity_type})")
        logger.info(f"   Muted: {bool(dialog.dialog.notify_settings.mute_until)}")
